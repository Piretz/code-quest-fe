...
import useloginSchema from '@/utils/validations/useLoginSchema';
  ...
  const handleSubmit = async (e: { preventDefault: () => void; }) => {
    ...
    if (!result.success) {
      ...
      try {
        ...
        const response = await axios.post('http://127.0.0.1:8000/api/user-student/login', {
          email,
          password,
        });
        ...
        <div className="flex flex-col items-start justify-center p-40 w-full h-full text-white font-poppins">
          ...
          <div className="fixed top-80 w-[450px] bg-opacity-70 p-10 rounded-lg">
            ...
            <div className="mb-6 relative">
              ...
              >
                {showPassword ? (
                  <i className="fas fa-eye text-white translate-y-8"></i> 
                ) : (
                  <i className="fas fa-eye-slash text-white translate-y-8"></i> 
                )}
              </button>
            </div>
...
>
  {loading ? (
    ...
    <div className="w-full h-full bg-teal-600 text-white text-center rounded-lg hover:scale-110 transition-transform">
      Login
    </div>
  )}
</button>
